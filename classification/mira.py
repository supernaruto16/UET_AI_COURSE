    ...
    def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, Cgrid):
        ...
        for C in Cgrid:
            ...
            for i in xrange(len(validationData)):
                predicted_score = util.Counter()
                for label in self.legalLabels:
                    predicted_score[label] = validationData[i] * self.weights[label]
                max_label = predicted_score.argMax()
                true_label = validationLabels[i]
                accuracy += 1 if max_label == true_label else 0        
            C_acc.append((accuracy, self.weights))
        
        C_acc.sort(reverse=True)
        self.weights = C_acc[0][1]
