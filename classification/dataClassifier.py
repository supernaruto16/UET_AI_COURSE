...
def enhancedPacmanFeatures(state, action):
    ...
    foods = next_state.getFood().asList()
    capsules = next_state.getCapsules()

    ghosts_distance = [util.manhattanDistance(ghost.getPosition(), pacman_pos)\
    ...
    # features['zoned_capsule_cnt'] = sum([1 if util.manhattanDistance(capsule, pacman_pos) < 3
    #                                     else 0 for capsule in capsules])
    # if len(capsules_distance) > 0:
    #     features['min_capsules_distance'] = min(capsules_distance)
